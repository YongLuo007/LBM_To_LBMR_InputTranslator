...
#    `functionName` is a function's name in your module (e.g., `landisToSpadesTranslatorEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/landisToSpadesTranslator/landisToSpadesTranslator/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("landisToSpadesTranslator")
  path <- list(modulePath = "C:/Users/yonluo/Documents/GitHub/landisToSpadesTranslator",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    landisToSpadesTranslator = list(.saveInitialTime = NA)
  )
  ...
  if (exists("landisToSpadesTranslatorEvent1", envir = .GlobalEnv)) {
    simOutput <- landisToSpadesTranslatorEvent1(mySim)
  } else {
    simOutput <- mySim$landisToSpadesTranslatorEvent1(mySim)
  }
  ...
  if (exists("landisToSpadesTranslatorEvent2", envir = .GlobalEnv)) {
    simOutput <- landisToSpadesTranslatorEvent2(mySim)
  } else {
    simOutput <- mySim$landisToSpadesTranslatorEvent2(mySim)
  }
